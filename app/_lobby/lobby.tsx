...
/** 가만히 이만큼 서 있으면 엉덩이를 긁고, 긁기가 끝나면 다시 센다 */
const SCRATCH_AFTER_MS = 3500;
const SCRATCH_MS = 2200;
const SCRATCH_FRAME_MS = 170;
...
/** 가만히 있던 시간 → 긁기 프레임. 손을 대고(1) 위(2)·아래(3)로 번갈아 긁는다 */
function scratchFrame(idleMs: number): CapybaraLook["scratch"] {
  const phase = idleMs - SCRATCH_AFTER_MS;
  if (phase < 0 || phase >= SCRATCH_MS) return 0;
  if (phase < SCRATCH_FRAME_MS) return 1;
  return Math.floor(phase / SCRATCH_FRAME_MS) % 2 === 0 ? 2 : 3;
}
/** 긁기가 끝나면 다시 처음부터 센다 */
const nextIdle = (idleMs: number, dt: number) => (idleMs + dt >= SCRATCH_AFTER_MS + SCRATCH_MS ? 0 : idleMs + dt);
...
export function Lobby({ games }: { games: DoorGame[] }) {
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      ...
      // 방향키가 우선, 안 누르고 있으면 누른 곳을 향해 걷는다
      ...
      if (wantsMove && !me.sitting) {
        ...
        const step = Math.min(WALK_SPEED * (dt / 1000), distance);
        ...
        me.facing = facingOf(dx, dy);
      }
      me.walkMs = moved ? me.walkMs + dt : 0;
      me.pose = moved ? walkPose(me.walkMs) : "stand";
      ...
      for (const remote of remotes.values()) {
        ...
        remote.walkMs = moving ? remote.walkMs + dt : 0;
    ...
    const draw = (now: number, door: Door | null, isStunned: boolean, attacking: boolean) => {
      ...
      for (const remote of remotes.values()) {
        const look: CapybaraLook = {
          pose: remote.walkMs > 0 ? walkPose(remote.walkMs) : "stand",
          sitting: remote.sitting,
          stunned: now < remote.stunUntil,
          attacking: now < remote.attackUntil,
          scratch: scratchFrame(remote.idleMs),
        };
      ...
      const myLook: CapybaraLook = {
        ...
        stunned: isStunned,
        attacking,
        scratch: scratchFrame(me.idleMs),
      };
      drawables.push({
    ...
    <>
      ...
      <div className="pointer-events-none absolute inset-x-0 bottom-0 flex flex-col items-center gap-2 px-4 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
        ...
        <p className="max-w-full text-balance rounded-lg bg-card/80 px-3 py-1.5 text-center text-caption-3 text-text-caption backdrop-blur">
          ...
          <span className="hidden [@media(pointer:coarse)]:inline">누른 곳으로 걸어가요 · 오두막 문 앞에 가면 입장</span>
        </p>
      </div>

      <div className="absolute bottom-[max(5rem,calc(env(safe-area-inset-bottom)+4rem))] right-4 flex flex-col items-end gap-3">
        {(seatNearby || sitting) && (
          <button
            type="button"
            onClick={() => {
            ...
            className="min-h-12 rounded-full bg-card/90 px-4 text-body-2 font-semibold text-text-strong shadow-md backdrop-blur focus-visible:outline-2 focus-visible:outline-primary"
          >
            {sitting ? "일어나기" : "통나무에 앉기"}
          </button>
        )}
        <button
          type="button"
          ...
          disabled={stunned}
          aria-label="때리기 (F)"
          className="size-16 rounded-full bg-card/90 text-body-2 font-bold text-text-strong shadow-md backdrop-blur focus-visible:outline-2 focus-visible:outline-primary disabled:opacity-50"
        >
          때리기
        </button>
      </div>
