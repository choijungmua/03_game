...
parser.add_argument("-m", "--model", help="Codex 모델 (기본: codex 설정값, 예: gpt-5.6-luna)")
parser.add_argument("--keep-background", action="store_true", help="배경 투명 처리를 건너뜀 (바닥 텍스처 등)")
args = parser.parse_args()

print(make_image(args.prompt, args.name, args.refs, transparent=not args.keep_background, model=args.model))
